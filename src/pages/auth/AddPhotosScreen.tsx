    ...
    <div className="min-h-screen bg-[#f7f5f0] flex flex-col items-center py-10 px-6">
      ...
      <div className="w-full max-w-sm flex flex-col flex-1 min-h-[calc(100vh-5rem)]">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-[14px] text-gray-500 text-center px-4 leading-snug">
            Add at least 2 photos so candidates can<br />get to know your authentic self.
          </p>
          {photos.length < 2 && (
            <span className="mt-3 text-[12px] font-semibold text-amber-800 bg-amber-100 px-3 py-1 rounded-full">
              📷 {2 - photos.length} more photo{2 - photos.length > 1 ? 's' : ''} required
            </span>
          )}
        </div>
        ...
        {/* Footer Section */}
        <div className="mt-auto pt-6 pb-4 w-full flex flex-col items-center gap-4">
          <Button variant="primary" onClick={handleContinue} disabled={photos.length < 2 || isSaving}>
            {isSaving ? 'Saving...' : photos.length < 2 ? 'Add at least 2 photos' : 'Continue'}
          </Button>
        </div>
