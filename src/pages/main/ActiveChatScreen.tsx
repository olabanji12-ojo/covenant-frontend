...
export const ActiveChatScreen = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#f7f5f0] w-full max-w-sm md:max-w-full mx-auto relative overflow-hidden">
      
      {/* Left Pane (Desktop Only) */}
      <div className="hidden md:flex w-[350px] lg:w-[400px] h-screen border-r border-gray-200 shrink-0 relative flex-col">
        <ChatList />
      </div>

      {/* Right Pane: Active Chat */}
      <div className="flex flex-col flex-1 h-screen relative bg-[#f7f5f0]">
      ...
      <div className="flex items-center justify-between px-4 pt-10 pb-4 bg-[#f7f5f0] z-10 shrink-0">
        ...
        <div className="flex items-center gap-4 text-gray-900 relative">
          ...
          {isDropdownOpen && (
            <div className="absolute top-10 right-0 w-48 bg-white rounded-[16px] shadow-lg border border-gray-100 overflow-hidden z-50">
              <button 
                onClick={() => { setIsDropdownOpen(false); setIsUnmatchOpen(true); }}
                className="w-full text-left px-5 py-4 text-[14px] font-bold text-gray-900 hover:bg-gray-50 transition-colors border-b border-gray-100"
              >
                Unmatch
              </button>
              <button 
                onClick={() => { setIsDropdownOpen(false); setIsReportOpen(true); }}
                className="w-full text-left px-5 py-4 text-[14px] font-bold text-red-500 hover:bg-red-50 transition-colors"
              >
                Report User
              </button>
            </div>
        ...
        {matchMessages.length === 0 ? (
          ...
          matchMessages.map((msg, index) => (
            <MessageBubble 
              ...
              time={new Date(msg.created_at || now).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
      ...
      {/* Bottom Input Area */}
      <div className="bg-[#f7f5f0] pt-2 pb-8 px-5 flex flex-col gap-4 shrink-0">
        
        {/* Suggested Icebreaker Tappable Chip (if history is empty and icebreaker exists) */}
        {matchMessages.length === 0 && matchUser?.icebreaker_prompt && (
          <button
            onClick={() => setInputText(`Hey ${matchName}! ${matchUser.icebreaker_prompt}`)}
            className="w-full bg-amber-100/80 hover:bg-amber-200/80 border border-amber-300/80 text-amber-950 p-2.5 rounded-2xl text-[12px] font-medium text-left flex items-start gap-2 transition-all shadow-xs"
          >
            <span className="text-base shrink-0">✨</span>
            <div className="flex flex-col">
              <span className="font-bold text-[10px] uppercase text-amber-800 tracking-wider">Tap to pre-fill Icebreaker</span>
              <span className="truncate">"Hey {matchName}! {matchUser.icebreaker_prompt}"</span>
            </div>
          </button>
        )}
      ...
      {matchUser?.id && (
        <ReportModal 
          ...
          onSuccess={() => {
            ...
            dispatch(apiSlice.util.invalidateTags(['Match'])); // Refresh matches
            navigate('/app/chat'); // Go back to inbox
          }}
        />
      )}
