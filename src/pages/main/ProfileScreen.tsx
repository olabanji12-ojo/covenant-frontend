...
import { ChevronLeft, Settings, CheckCircle2, Crown, LogOut, User, ShieldAlert } from 'lucide-react';
...
export const ProfileScreen = () => {
  ...
  const age = React.useMemo(() => {
    ...
    if (!dobString || dobString.startsWith('0001')) return 'N/A'; // Fallback for missing or invalid DOB
  ...
  const name = user?.first_name ? `${user.first_name}`.trim() : (user?.email ? user.email.split('@')[0] : 'User');
  const avatar = user?.photos && user.photos.length > 0 ? user.photos[0] : '/male1.png';

  return (
    <div className="flex flex-col min-h-screen bg-[#f7f5f0] w-full max-w-sm md:max-w-2xl mx-auto relative overflow-hidden pb-28 md:pb-6 md:bg-white md:shadow-sm md:border-x border-gray-100">
        ...
        <div className="flex flex-col w-full px-8 pb-6">
          ...
          */}
          ...
          <SettingsRow 
            ...
            onClick={() => navigate('/app/privacy-safety')}
          />
          {/*
          <SettingsRow 
