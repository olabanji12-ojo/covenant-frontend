...
export const socialLoginUser = createAsyncThunk(
  ...
  async ({ provider, token: socialToken }: { provider: string; token: string }, { rejectWithValue }) => {
    try {
      const data = await AuthService.socialLogin(provider, socialToken);
      localStorage.setItem('token', data.token);
      return data;
    } catch (error: unknown) {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // Social Login
    builder.addCase(socialLoginUser.pending, (state) => {
      state.isLoading = true;
      state.error = null;
    });
    builder.addCase(socialLoginUser.fulfilled, (state, action: PayloadAction<AuthResponse>) => {
      state.isLoading = false;
      state.isAuthenticated = true;
      state.user = action.payload.user;
      state.token = action.payload.token;
    });
    builder.addCase(socialLoginUser.rejected, (state, action) => {
      state.isLoading = false;
      state.error = action.payload as string;
    });

    // Guest Login
    builder.addCase(guestLoginUser.pending, (state) => {
      state.isLoading = true;
      state.error = null;
    });
    builder.addCase(guestLoginUser.fulfilled, (state, action: PayloadAction<AuthResponse>) => {
      state.isLoading = false;
      state.isAuthenticated = true;
      state.user = action.payload.user;
      state.token = action.payload.token;
    });
    builder.addCase(guestLoginUser.rejected, (state, action) => {
      state.isLoading = false;
      state.error = action.payload as string;
    });
